...
def economy_plan(state: Snapshot, memory: ExplorationMemory | None = None) -> Plan:
    ...
    resource_candidates: list[tuple[int, str, Position, Unit]] = []
    for worker in empty:
        for resource in memory.visible_targets(state):
            path = plan_path(worker.position, {resource}, obstacles, set(positions.values()) - {worker.position})
            if path.status in {"FOUND", "START_AT_GOAL"}:
                resource_candidates.append((path.path_length, worker.id, resource, worker))
    for _, _, resource, worker in sorted(resource_candidates):
        if worker.id in desired or resource in resource_reserved:
            continue
        resource_reserved.add(resource)
        desired[worker.id] = (worker, resource, "RESOURCE")
